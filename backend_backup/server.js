...
// IPFS configuration from env or local default
...
const KEY_PATH = process.env.KEY_PATH ? path.resolve(__dirname, process.env.KEY_PATH) : null;
...
// For local test-network, keys are in a 'keystore' directory with a hash name
...
async function initFabric() {
    try {
        ...
        if (KEY_PATH && fs.existsSync(KEY_PATH)) {
            privateKeyPem = fs.readFileSync(KEY_PATH).toString();
        } else if (fs.existsSync(KEY_DIR_PATH)) {
            ...
            throw new Error(`Private key not found at ${KEY_PATH || KEY_DIR_PATH}`);
        }
